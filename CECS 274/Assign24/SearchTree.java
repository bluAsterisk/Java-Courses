...
class SearchTree <KeyType extends Comparable<KeyType> & HasClone <KeyType>, DataType extends HasVisit<DataType> & HasClone<DataType>>
{
...
public SearchTree(SearchTree<KeyType, DataType> other)
{
  CopyOf(other);
};
...
public void CopyOf(SearchTree<KeyType, DataType> B)
{
  Root = CopyOf(B.Root);
};
...
private boolean Balanced(Tree_Node B)
{
  if(B == null)
    return true;
  else if((Math.abs(Height(B.Left) - Height(B.Right)) <= 1))
    return Balanced(B.Left) && Balanced(B.Right);
  else
    return false;
};

private Tree_Node CopyOf(Tree_Node B)
{
  if(B == null)
	 return null;
  else
  {
    ...
    copy.Key = B.Key;
    copy.Data = B.Data;
    copy.Left = CopyOf(B.Left);
    copy.Right = CopyOf(B.Right);
    return copy;
  }
};
...
private void Report(Tree_Node T, int counter)
{
  ...
  if (T != null)
  {
    Report(T.Left, counter + 1);
    for(int i = 0; i < counter; i++)
	  System.out.print("*");
    T.Data.Visit();
    Report(T.Right, counter + 1);
  }
};
